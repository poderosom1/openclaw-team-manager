...
async function addFeishuBot(): Promise<void> {
  ...
  console.log(chalk.bold('➕ 为 Agent 添加飞书 Bot'));
  console.log(chalk.bold.cyan('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n'));
  
  console.log(chalk.bold.yellow('📖 详细教程：'));
  console.log(chalk.cyan('  https://www.feishu.cn/content/article/7602519239445974205\n'));
  
  console.log(chalk.bold.yellow('📋 前置准备（在飞书开放平台完成）：\n'));
  console.log(chalk.bold('步骤 1：创建企业自建应用'));
  console.log(chalk.dim('  • 进入飞书开放平台：https://open.feishu.cn/app'));
  console.log(chalk.dim('  • 点击「创建企业自建应用」，填写应用信息\n'));
  
  console.log(chalk.bold('步骤 2：启用机器人能力'));
  console.log(chalk.dim('  • 进入「应用能力」→「机器人」'));
  console.log(chalk.dim('  • 开启机器人能力，设置机器人名称\n'));
  
  console.log(chalk.bold('步骤 3：配置应用权限'));
  console.log(chalk.dim('  • 进入「权限管理」→ 点击「批量导入」'));
  console.log(chalk.dim('  • 复制以下 JSON 内容直接导入：\n'));
  console.log(chalk.cyan('  {'));
  console.log(chalk.cyan('    "scopes": {'));
  console.log(chalk.cyan('      "tenant": ['));
  console.log(chalk.cyan('        "contact:user.base:readonly",'));
  console.log(chalk.cyan('        "im:message",'));
  console.log(chalk.cyan('        "im:message.p2p_msg:readonly",'));
  console.log(chalk.cyan('        "im:message.group_at_msg:readonly",'));
  console.log(chalk.cyan('        "im:message:send_as_bot",'));
  console.log(chalk.cyan('        "im:resource"'));
  console.log(chalk.cyan('      ]'));
  console.log(chalk.cyan('    }'));
  console.log(chalk.cyan('  }\n'));
  console.log(chalk.dim('  提示：可根据实际需求添加更多权限\n'));
  
  console.log(chalk.bold('步骤 4：获取凭证'));
  console.log(chalk.dim('  • 进入「凭证与基础信息」页面'));
  console.log(chalk.dim('  • 复制 App ID 和 App Secret\n'));
  console.log(chalk.bold.cyan('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n'));
  ...
  console.log(chalk.bold.cyan('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));
  console.log(chalk.bold.yellow('📋 后续步骤：\n'));
  
  console.log(chalk.bold('步骤 1：启动 Gateway'));
  console.log(chalk.cyan('  openclaw gateway'));
  console.log(chalk.dim('  ⚠️ 必须先启动 Gateway，否则长连接配置会失败\n'));
  
  console.log(chalk.bold('步骤 2：配置事件订阅（飞书开放平台）'));
  console.log(chalk.dim('  • 进入「事件订阅」页面'));
...
async function showPairingMenu(): Promise<void> {
  ...
  while (true) {
    ...
    if (requests.length === 0) {
      ...
      console.log(chalk.dim('提示：用户在飞书私聊机器人后，会收到配对码\n'));
      await tuiUtils.waitForKey();
      return;
    }
    ...
    try {
      const output = execSync(command, { 
        encoding: 'utf-8',
        cwd: getOpenClawRoot() || undefined
      });
      
      console.log(chalk.green('\n✓ 配对授权成功！\n'));
      console.log(chalk.dim(output));
      console.log(chalk.yellow('提示：用户现在可以在飞书中与机器人对话。\n'));
      ...
      console.log(chalk.dim(`错误信息: ${errorMsg}\n`));
      
      console.log(chalk.dim('可能原因：'));
      console.log(chalk.dim('  • 配对码无效或已过期'));
      console.log(chalk.dim('  • 配对码已被使用'));
      console.log(chalk.dim('  • Gateway 未运行'));
      console.log(chalk.dim('  • openclaw 命令未安装或不在 PATH 中\n'));
    }
 ...
 */
function approveFeishuPairing(code: string, accountId?: string): { success: boolean; error?: string; userId?: string; targetAccountId?: string } {
...
async function approvePairingRequest(): Promise<void> {
  ...
  if (result.success) {
    ...
    console.log(chalk.dim(`账号: ${result.targetAccountId}\n`));
    console.log(chalk.yellow('提示：用户现在可以在飞书中与机器人对话。\n'));
  } else {
    console.log(chalk.red('\n✗ 配对授权失败\n'));
