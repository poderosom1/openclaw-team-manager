...
export class SetupService {
  ...
  async init(assistantName: string = '总助理', openClawRoot?: string): Promise<{ success: boolean; message: string; assistant_id: string }> {
    try {
      ...
      // 4. Configure Gateway (allow TUI tools to connect)
   ...
   * Configure Gateway
   * 
   * Auto-configures gateway.controlUi.allowInsecureAuth: true
   * To allow TUI tools to connect Gateway via WebSocket
   */
  private configureGateway(): void {
    const openclawJsonPath = require('../utils').getOpenClawJsonPath();
    ...
    try {
      ...
      // If allowInsecureAuth already configured, skip
      if (config.gateway.controlUi.allowInsecureAuth === true) {
        return;
      }

      // Set allowInsecureAuth
      const oldValue = config.gateway.controlUi.allowInsecureAuth;
      config.gateway.controlUi.allowInsecureAuth = true;

      // Write back config file
      fs.writeFileSync(openclawJsonPath, JSON.stringify(config, null, 2), 'utf-8');
      console.log(chalk.dim(`  已配置 Gateway 允许 TUI 连接`));

      // Record change to config_changes
      configTracker.recordGatewayConfigChange(
        'gateway.controlUi.allowInsecureAuth',
        oldValue,
        true,
        '允许 TUI 工具通过 WebSocket 连接 Gateway'
      );
    } catch (error) {
      console.warn(`警告: 配置 Gateway 失败: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
