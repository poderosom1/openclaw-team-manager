...
export class AgentService {
  ...
  create(data: {
    ...
    try {
      ...
      if (agent.department_id) {
        const allowListResult = updateAgentAllowList(agentId, agent.department_id, 'add');
  ...
  delete(id: string): { success: boolean; message: string; restartReminder?: string } {
    ...
    if (agent.department_id) {
      const allowListResult = updateAgentAllowList(id, agent.department_id, 'remove');
      if (allowListResult.success && allowListResult.affectedAgents.length > 0) {
    ...
    const cleanupResult = cleanupFeishuCredentialsForAgent(id);
