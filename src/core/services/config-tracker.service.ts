...
export type ChangeType = 'agent_create' | 'agent_delete' | 'agent_allow_list_update' | 'binding_create' | 'binding_delete' | 'channel_config' | 'skill_pack_create' | 'skill_pack_delete' | 'job_create' | 'job_delete' | 'directory_create' | 'config_update' | 'feishu_group_bind' | 'feishu_group_unbind' | 'feishu_pairing_approve' | 'other';
export type TargetType = 'agents.list' | 'agents.list.allow_agents' | 'bindings' | 'channels.feishu' | 'channels.feishu.groups' | 'skill_packs' | 'jobs' | 'directories' | 'gateway' | 'departments' | 'credentials' | 'other';
...
/**
 * Record agent allow list change
 * 
 * Records changes to agents.list[].subagents.allowAgents
 */
export function recordAgentAllowListChange(
  agentId: string,
  oldAllowList: string[],
  newAllowList: string[],
  reason: string
...
/**
 * Revert agent allow list changes
 */
function revertAgentAllowListChange(config: Record<string, unknown>, change: configChangeRepo.ConfigChange): void {
  if (!config.agents) config.agents = { list: [] };
  const agents = config.agents as Record<string, unknown>;
  if (!agents.list) agents.list = [];
  ...
  const agentConfig = agentsList.find((a: Record<string, unknown>) => a.id === agentId);
  ...
  if (change.old_value) {
    try {
      ...
      const subagents = agentConfig.subagents as Record<string, unknown> | undefined;
      if (!subagents) {
